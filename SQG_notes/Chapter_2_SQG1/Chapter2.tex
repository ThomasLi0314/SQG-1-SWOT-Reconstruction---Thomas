...
\section{\texorpdfstring{$\QGone$}{QG1} Model}
    ...
    Then after scaling anlysis we have \mn{\color{red} Check the Dimension Here!!!!!}
    \[Q = N^2 f + \epsilon q\]
    where 
    \begin{equation}
...
\begin{align}
    \mcalL(\Phi_{sur}^1) &= 0 \\
    \Phi_{sur,z}^{1,t} &= C_b - \partial_z \Phi_{int}^{1,t} = C_b - \frac{f}{\Bur N^2}\Phi_z^{0,t}\Phi_{zz}^{0,t}
\end{align}
