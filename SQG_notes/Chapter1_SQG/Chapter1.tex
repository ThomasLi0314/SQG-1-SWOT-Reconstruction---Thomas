...
(\widehat{u}, \widehat{v})=U^{-1}(u, v) \quad \widehat{w}=\frac{L}{U H} w, \quad \widehat{f}=f_0^{-1} f, \quad \widehat{\phi}=\frac{\phi_1}{f_0 U L}, \quad \widehat{b}=\frac{H}{f_0 U L} b_1
    ...
    Then 
    \[ Q = \underbrace{fN^2}_{\text{Background}} + \underbrace{(N^2\xi + fb_z)}_{\text{Linear terms}} + \underbrace{(\xi b_z - v_zb_x + u_zb_y)}_{\text{Nonlinear terms}}\]
    Use the vector identity 
    \[ \nabla_3 \cdot (\bm{\omega}_a b_{\text{tot}}) = \bm{\omega} 、\cdot \nabla_3 \btot + \underbrace{\btot (\nabla \cdot \bm{\omega})}_{=0, \bm{\omega} = \nabla \times \cdot}\]
    Then after scaling anlysis we have 
    \[Q = N^2 f + \epsilon q\]
    where 
    \[ q = N^2 \xi + \frac{1}{\Bur} fb_z + \frac{\epsilon}{\Bur} \nabla_3 (\bm{\omega} \btot) \]
