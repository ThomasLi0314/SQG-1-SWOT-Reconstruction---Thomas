...
function [u_surface, v_surface] = calculate_surface_u(phi0_s_hat, mu, inv_mu, kx, ky, K2, inv_K2, epsilon, Bu)
    ...
    phi0_s_zz = ifft2(phi0_s_hat .* mu .* mu);
    phi0_s_yz = ifft2(phi0_s_hat .* (1i) .* ky .* mu);
    phi0_s_z = ifft2(phi0_s_hat .* mu);
    ...
    phi0_s_xz = ifft2(phi0_s_hat .* (1i) .* kx .* mu);

    G1_term1 = fft2(phi0_s_x .* phi0_zz + phi0_s_xz .* phi0_s_z);
    ...
    u_surface = ifft2(u_s_hat);
    v_surface = ifft2(v_s_hat);
end